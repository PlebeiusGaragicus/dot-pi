/**
 * Damage Control — YAML-rule safety auditing for tool calls
 *
 * Loads rules from .pi/damage-control-rules.yaml to enforce guardrails:
 *   - bashToolPatterns: regex patterns to block/confirm dangerous bash commands
 *   - zeroAccessPaths: paths the agent cannot read or write
 *   - readOnlyPaths: paths the agent can read but not write
 *   - noDeletePaths: paths where rm/delete is blocked
 *
 * Intercepts tool_call events and either blocks, prompts for confirmation,
 * or allows based on the loaded rules.
 *
 * Usage: pi -e extensions/safety/damage-control.ts
 */
