...
This is different from the repo-level `core/bin/` directory. `dotpi sync` creates command symlinks such as `core/bin/reader -> ../../dispatch-agent` so agent names can be run directly from the shell. It also adds **`core/bin/todo` → `../utilities/todo/todo`** (the JSONL todo CLI) unless an agent config directory is literally named `agents/todo`, which reserves `core/bin/todo` for that dispatcher.
...
```text
core/bin/reader -> ../../dispatch-agent
core/bin/web -> ../../dispatch-agent
core/bin/todo -> ../utilities/todo/todo
```

For agent symlinks, the name determines which `agents/<name>/` config root `dispatch-agent` launches. **`todo`** is the exception: it runs the standalone todo utility (see `core/utilities/todo/README.md`).
