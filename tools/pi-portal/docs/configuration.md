...
|----------|-------------|
| `DOT_PI_DIR` or `DOT_PI_ROOT` | Absolute path to the **dot-pi** repository root (must contain `agents/`, `shared/`, `setup.sh`). If unset, defaults to **two levels above** the pi-portal package directory (so `dot-pi` is resolved when this repo lives at `dot-pi/tools/pi-portal`). |
