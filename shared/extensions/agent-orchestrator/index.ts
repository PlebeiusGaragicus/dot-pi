...
function buildUsageCatalog(agents: AgentConfig[]): string | null {
	...
	const sections = agents
		...
		.map((agent) => {
			const body = agent.usage?.trim() || agent.description;
			return `### ${agent.name}\n\n${body}`;
		});
	...
	return [
		"## Available Subagents",
		"",
		"The following subagents are available through the `subagent` tool.",
		"",
		...sections,
	].join("\n");
}
...
export default function (pi: ExtensionAPI) {
	if (!process.env.PI_IS_SUBAGENT) {
		pi.on("before_agent_start", async (event) => {
			...
			const usageCatalog = buildUsageCatalog(discovery.agents);
			if (!usageCatalog) return;
			return { systemPrompt: `${event.systemPrompt}\n\n${usageCatalog}` };
		});
	}

	pi.registerCommand("subagents", {
		description: "Show subagents available to this agent",
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) return;
			const discovery = discoverAgents(ctx.cwd, "user");
			if (discovery.agents.length === 0) {
				ctx.ui.notify("Subagents\n\nNo subagents are available from this agent's local agents/ directory.", "info");
				return;
			}
	...
	pi.registerTool({
		...
		description: [
			...
			'Default agent scope is "user" (from the active agent config\'s agents/ directory).',
