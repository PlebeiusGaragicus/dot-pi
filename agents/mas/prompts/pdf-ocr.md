...
Use top-level capability agents to ingest a PDF (or acquire one from a URL), render each page to an image, OCR each page into markdown, optionally audit uncertain pages, and optionally assemble a full document or summary. This workflow matches the quality bar of the legacy **reader** MAS: durable artifacts under `pages/`, a `reader-manifest.json` index, page-by-page transcription with explicit uncertainty markers, and resumable progress when images already exist.

## Goal

Produce auditable OCR from a user-supplied **file path or HTTPS URL**. Prefer saved files (`pages/*.md`, optional `document.md` / `summary.md`) over pasting full page text into chat unless the user explicitly asks for inline output.
...
- If `reader-manifest.json` exists **and** `pages/` contains at least one `page-*.png`, treat the workspace as **resumable** unless the user explicitly requested re-ingestion. In that case, skip to **Phase 5** (identify pages still needing OCR).
...
3. Prefer Poppler: `pdfinfo` for page count and `Page size:` (points); choose DPI per sizing rules; render with `pdftoppm -r <dpi> -png <pdf> pages/page` and normalize names to `pages/page-0001.png`, etc.
4. If Poppler is missing, try ImageMagick `magick` with equivalent DPI logic.
...
6. Skip re-rendering existing `pages/page-*.png` unless the user asked for re-ingestion.
7. Write `reader-manifest.json` at the workspace root using valid JSON with this shape:
   - `source_pdf`, `source_pdf_resolved`, `ingested_at` (ISO 8601), `page_count`, `render` (format, dpi, target_max_px, command, renderer, resized), `pages[]` each with `page`, `image`, `markdown`, `status`, `ocr_status` (start as `pending` for rendered pages), `notes`, and top-level `issues[]`.
...
Identify every page that still needs OCR: missing `pages/page-NNNN.md`, or prior run left `ocr_status` / worker notes indicating `pending` or `failed`.
...
Inputs:
- Page number: <n>
- Image path: pages/page-<zero-padded n>.png
- Output markdown path: pages/page-<zero-padded n>.md
...
5. Do not update `reader-manifest.json`.
...
4. Do not update `reader-manifest.json`.
...
- Read every `pages/page-*.md` in lexical order.
- If the user asked for a full transcript: write `document.md` starting with the title `# OCR Document`, a line `Source manifest: reader-manifest.json`, a short HTML comment that content was transcribed from images, then `## Page N` sections in order with page body content. Do not strip `[unclear: ...]` markers or warnings.
- If the user asked for a summary: write `summary.md` from the OCR text; note pages with needs_review or low confidence.
- Do not modify files under `pages/`.
...
Use your own `read` (and `ls` / `find` if needed) to confirm `reader-manifest.json` exists, `pages/page-*.md` covers expected page range, and any promised `document.md` / `summary.md` exist.
...
- `pages/page-0001.png`, `pages/page-0001.md`, … — page images and transcripts.
- `reader-manifest.json` — ingestion metadata and page index.
...
`OCR complete. Pages under ./pages/; manifest ./reader-manifest.json.`
