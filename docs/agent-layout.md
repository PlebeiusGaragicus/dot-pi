...
This is different from the repo-level `core/bin/` directory. `dotpi sync` creates command symlinks such as `core/bin/reader -> ../../dispatch-agent` so agent names can be run directly from the shell.
