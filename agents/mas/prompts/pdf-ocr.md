...
- If `reader-manifest.json` exists **and** `pages-png/` contains at least one `page-*.png`, treat the workspace as **resumable** unless the user explicitly requested re-ingestion. In that case, skip to **Phase 5** (identify pages still needing OCR).
...
Use your own `read` (and `ls` / `find` if needed) to confirm `reader-manifest.json` exists, `pages-ocr/page-*.md` covers expected page range, and any promised `document.md` / `summary.md` exist.
...
`OCR complete. PNGs under ./pages-png/; markdown under ./pages-ocr/; manifest ./reader-manifest.json.`
