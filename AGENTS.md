...
**Rebuilding symlinks**: Run `dotpi sync` to rebuild `core/bin/` symlinks, ensure `shared/` symlinks to `~/.pi/agent/` exist, and link each `agents/<name>/auth.json` → `shared/auth.json`, after adding or removing agent configs.
