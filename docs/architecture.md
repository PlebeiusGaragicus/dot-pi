...
Prompt templates are orchestration policy, not capability grants. Each delegated step must match the target worker's structural tools, skills, and artifact contract. See [Workflow Writing Guide](workflow-writing-guide.md) for the condensed checklist and [Multi-Agent Systems](reference/multi-agent-systems.md#workflow-authoring-and-capability-boundaries) for detailed authoring rules and examples.
